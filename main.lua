...
--// Basé sur l'interface Fluent (Dawid Scripts)
...
local states = {
    ...
    -- États des consommables automatiques
...
local function tryLoadFromMemoryScan()
    -- Scan ultra-profond de la table de registre Lua et du Garbage Collector
...
-- Fonction de recherche multi-ID pour consommables
local function getInventoryItems(searchName, targetCategory)
    ...
    if not inventory then 
        print("[Premium Hub Debug] Impossible de lire l'inventaire.")
        return {} 
    end
...
-- ==========================================
local Tabs = {
    Main = Window:AddTab({ Title = "Mouvements", Icon = "plane" }),
    Visuals = Window:AddTab({ Title = "Visuels & ESP", Icon = "eye" }),
    Farm = Window:AddTab({ Title = "Farming", Icon = "axe" }),
    Eggs = Window:AddTab({ Title = "Oeufs", Icon = "box" }),
    Event = Window:AddTab({ Title = "Événements", Icon = "sparkles" }),
    Consumables = Window:AddTab({ Title = "Consommables", Icon = "flask" }),
    Quests = Window:AddTab({ Title = "Quêtes", Icon = "clipboard" }),
...
Tabs.Farm:AddParagraph({ Title = "✨ Auto Tap Aura", Content = "Détruit automatiquement les objets autour de toi." })
...
auraVisual.Color = Color3.fromRGB(80, 255, 120)
...
Tabs.Farm:AddParagraph({ Title = "💰 Auto Collect", Content = "Récupère instantanément tous les orbes sur la carte." })

local AutoCollectToggle = Tabs.Farm:AddToggle("AutoCollectToggle", {Title = "Activer Auto Collect", Default = false })
...
Tabs.Farm:AddParagraph({ Title = "🚪 Auto Buy Zones", Content = "Achète automatiquement la prochaine zone." })

local AutoBuyZoneToggle = Tabs.Farm:AddToggle("AutoBuyZoneToggle", {Title = "Activer Auto Buy Zone", Default = false })
...
-- AUTO MINE
Tabs.Farm:AddParagraph({ Title = "⛏️ Auto Mine (Digsite)", Content = "Minage sans lag avec téléportation automatique à l'entrée." })
...
AutoMineToggle:OnChanged(function(state)
    ...
    if states.autoMine then
        -- Téléportation instantanée à l'entrée du Digsite quand on active le minage !
        pcall(function()
...
Tabs.Eggs:AddParagraph({ Title = "🥚 Auto Open Eggs", Content = "Ouvre automatiquement l'œuf sélectionné et supprime l'animation pour éviter les lags." })
...
local DisableAnimToggle = Tabs.Eggs:AddToggle("DisableAnimToggle", {Title = "Désactiver Animations d'Œufs (No Lag)", Default = false })
...
Tabs.Event:AddParagraph({ Title = "🛡️ Lucky Defense Minigame", Content = "Achète automatiquement les améliorations que tu as sélectionnées." })
...
-- Scan dynamique toutes les 5s : Ajoute de nouvelles améliorations au menu si le jeu en crée
task.spawn(function()
    local knownUpgrades = {}
...
-- ==========================================
-- 8. LOGIQUE DES CONSOMMABLES AUTOMATIQUES (POTIONS & FLAGS)
-- ==========================================

Tabs.Consumables:AddParagraph({ Title = "🧪 Auto Potions", Content = "Consomme en boucle tes potions sélectionnées toutes les 5 secondes (sans restriction de doublons)." })
    ...
    if states.autoConsumePotions then
        task.spawn(function()
            while states.autoConsumePotions do
                    ...
                    if potionRemote then
                        for potionType, active in pairs(states.selectedPotions) do
                            if active and states.autoConsumePotions then
                                -- On récupère TOUTES les potions correspondantes (tous les tiers et GUIDs)
                                local matches = getInventoryItems(potionType, "Potion")
                                for _, item in ipairs(matches) do
                                    if states.autoConsumePotions then
                                        print("[Premium Hub] Utilisation potion : " .. tostring(item.realName) .. " | GUID: " .. tostring(item.guid))
                                        potionRemote:FireServer(item.guid, 1)
                                        task.wait(0.1) -- Petit délai pour éviter le spam kick
                                    end
                                end
                            end
                        end
                ...
                task.wait(5) -- Utilise à intervalle régulier de 5 secondes quoi qu'il arrive
            end
        end)
    end
end)

Tabs.Consumables:AddParagraph({ Title = "🚩 Auto Place Flags", Content = "Plante automatiquement le drapeau choisi toutes les 5 secondes." })
...
FlagsDropdown:OnChanged(function(Value)
    states.selectedFlag = Value
end)
            ...
            while states.autoPlaceFlags do
                pcall(function()
                    if states.selectedFlag ~= "" then
                        ...
                        if flagRemote then
                            -- On cherche tous les drapeaux de ce type
                            local matches = getInventoryItems(states.selectedFlag, "Flag")
                            if #matches > 0 then
                                -- On trie pour utiliser le meilleur drapeau disponible
                                table.sort(matches, function(a, b) return a.tier > b.tier end)
                                local bestFlag = matches[1]
                                print("[Premium Hub] Placement drapeau : " .. tostring(bestFlag.realName) .. " | GUID: " .. tostring(bestFlag.guid))
                                flagRemote:InvokeServer(bestFlag.realName, bestFlag.guid)
                            end
                        end
                    end
                end)
                task.wait(5) -- Pose un drapeau toutes les 5 secondes en continu
            end
        end)
    end
end)
...
Tabs.Quests:AddParagraph({ Title = "🤖 Auto Quests Autopilot", Content = "Analyse et exécute simultanément vos quêtes actives en arrière-plan !" })

-- Fonction de scan des quêtes actives dans l'interface GoalsSide
...
-- Scanner de diamants robustes basés sur rbxassetid
    ...
    if breakablesFolder then
        for _, group in pairs(breakablesFolder:GetChildren()) do
            for _, breakable in pairs(group:GetChildren()) do
                ...
                -- Recherche récursive dans les textures du bloc
                ...
                -- Fallback sur le nom en cas de doute
...
-- Variables de suivi pour savoir ce que le système de quêtes a activé en arrière-plan
local questEnabledEgg = false
local questEnabledPotions = false
local questEnabledMine = false

local AutoQuestsToggle = Tabs.Quests:AddToggle("AutoQuestsToggle", {Title = "Activer l'Autopilot", Default = false })
    ...
    if states.autoQuests then
        task.spawn(function()
            while states.autoQuests do
                ...
                -- Analyse textuelle de chaque quête en cours
                ...
                -- 🚀 1. TÂCHE PARALLÈLE : ÉCLOSION DES OEUFS (SANS BLOQUER LE RESTE !)
                if hasHatchQuest then
                    if not questEnabledEgg then
                        ...
                        pcall(function()
                            ...
                            if #displayList > 0 then
                                -- Choisit l'oeuf 20 ou le plus élevé disponible
                        ...
                        if AutoEggToggle then
                            AutoEggToggle:SetValue(true)
                        else
                            states.autoEgg = true
                        end
                    end
                    ...
                    if questEnabledEgg then
                        ...
                        if AutoEggToggle then
                            ...
                            states.autoEgg = false
                        end
                    end
                end
                
                -- 🚀 2. TÂCHE PARALLÈLE : UTILISATION DES POTIONS (SANS BLOQUER !)
                if hasPotionQuest then
                    if not questEnabledPotions then
                        questEnabledPotions = true
                        -- On s'assure qu'au moins quelques potions de base soient actives si aucune n'est cochée
                        ...
                        if AutoPotionsToggle then
                            AutoPotionsToggle:SetValue(true)
                        else
                            states.autoConsumePotions = true
                        end
                    end
                else
                    if questEnabledPotions then
                        ...
                        if AutoPotionsToggle then
                            ...
                            states.autoConsumePotions = false
                        end
                    end
                end
                
                -- 🚀 3. TÂCHE PHYSIQUE (SÉQUENTIELLE) : POSITIONNEMENT DU PERSONNAGE
                -- Les quêtes Diamant et Digsite demandent des téléportations physiques, on les traite par ordre de priorité.
                if hasDiamondQuest then
                    -- Si on était en train de miner au Digsite, on met d'abord le minage sur pause pour éviter les conflits de téléportation
                    ...
                    if breakable and hitbox then
                        ...
                        -- Pas de diamant actif sur la carte ? Téléportation à la zone Underworld en attendant
                        pcall(function()
                ...
                elseif hasMineQuest then
                    -- Si la quête de diamant est finie mais qu'on a une quête mine, on lance l'Auto Mine
                    if not questEnabledMine then
                        questEnabledMine = true
                        states.digsiteMode = "Efficiency" -- Passage auto en mode efficacité
                        if AutoMineToggle then
                            AutoMineToggle:SetValue(true)
                        else
                            states.autoMine = true
                        end
                    end
                else
                    -- Plus aucune quête physique active, on arrête le minage automatique
                    if questEnabledMine then
                        questEnabledMine = false
                        if AutoMineToggle then AutoMineToggle:SetValue(false) else states.autoMine = false end
                    end
        ...
        -- Protection : Désactivation propre de toutes les activités lancées par l'autopilot
        if questEnabledEgg then
            questEnabledEgg = false
...
-- Affichage en temps réel des quêtes dans l'onglet Quêtes
task.spawn(function()
    while task.wait(1.5) do
        ...
        pcall(function()
            if states.autoQuests then
                QuestDisplayParagraph:SetTitle("🟢 Tracker Auto Quests Actif")
            else
                QuestDisplayParagraph:SetTitle("🔴 Tracker Auto Quests Désactivé")
            end
...
Tabs.Settings:AddParagraph({ Title = "⚙️ Utilitaires", Content = "Options pratiques pour ton confort de jeu." })

local afkConnection
local AntiAfkToggle = Tabs.Settings:AddToggle("AntiAfkToggle", {Title = "Mode AFK (Anti-Déconnexion)", Default = false })
...
Tabs.Settings:AddButton({
    ...
    Description = "Désactive tout et supprime l'interface.",
    Callback = function()
...
Fluent:Notify({
    Title = "Premium Hub",
    Content = "Le Hub a été mis à jour avec l'Autopilot multi-tâches !",
    SubContent = "Appuie sur RightControl pour cacher le menu",
    Duration = 5
})
